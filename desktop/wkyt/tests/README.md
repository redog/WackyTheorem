...
The walkthrough covers a mixed file/Calendar query, overlapping saved views, automatic live refresh after source correction, pinned original payloads, encrypted-vault reopen, persisted definitions, removing a view while retaining both sources, and source deletion/restoration without orphaned generated claims or links. Failures stop the run; no screenshot or page dump is taken during the recovery ceremony. Optional `--screenshot /absolute/path.png` captures synthetic results only after all checks pass.
